...
flowchart LR
    ...
    A["api_facturas<br/>C# / ASP.NET Core — :8054"]
    B[("PostgreSQL<br/>bdfacturas COMPLETA, dada<br/>:15454")]
...
En una frase: *alguien le habla JSON a la API por el 8054, y la API le
...
1. **`docker compose up -d --build` — un solo comando —** deja corriendo
   ...
   `GET http://localhost:8054/` responde el JSON de diagnóstico. Guardar un
